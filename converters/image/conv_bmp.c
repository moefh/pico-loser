...
static int conv_file(struct INFO *info)
{
  ...
  fprintf(out, "#define img_%s_width   %d%s", info->var_name, reader.w, line_end);
  fprintf(out, "#define img_%s_height  %d%s", info->var_name, reader.h, line_end);
...
static void print_help(const char *progname)
{
  ...
  printf("   -num-tiles-x     set the number of horizontal tiles\n");
  printf("   -num-tiles-y     set the number of vertical tiles\n");
  printf("   -esp32-scramble  don't scramble image as required by ESP32 I2S\n");
...
static int parse_args(struct INFO *info, int argc, char *argv[])
{
  ...
  for (int i = 1; i < argc; i++) {
    if (argv[i][0] == '-') {
      ...
      } else if (strcmp(argv[i], "-esp32-scramble") == 0) {
        info->scramble_for_esp32 = 1;
